...
city_to_suburb <- complete_trip_2007 %>%
  filter(origin_ent == "09" & dest_ent %in% c("13","15")) %>%
...
complete_suburb_city_results <- classify_trips(complete_suburb_city)


complete_suburb_city_results <- complete_suburb_city_results %>%
  rowwise() %>%
  mutate(
...
suburb_city_summary <- complete_suburb_city_results %>%
  group_by(mode_key) %>%
  summarise(
...
sum(suburb_city_summary$weighted_count)
# write.csv(suburb_city_summary, "data/city-city-suburb/2007_city_suburb_mode_summary_weighted.csv", row.names = FALSE)
